    ...
    async (audioBlob: Blob) => {
      ...
      try {
        ...
        if (typeof window !== 'undefined') {
          ...
          // Intercept Puter ASR to use client-side adapter entirely
          if (asrProviderId === 'puter-asr') {
            const { transcribeWithPuterASR } = await import('@/lib/audio/puter-audio-adapter');
            const providerConfig = asrProvidersConfig?.[asrProviderId];
            const modelId = providerConfig?.modelId || ASR_PROVIDERS[asrProviderId]?.defaultModelId || 'gpt-4o-mini-transcribe';
            
            // Convert Blob to File so Puter can detect format
            const file = new File([audioBlob], 'recording.webm', { type: 'audio/webm' });
            const result = await transcribeWithPuterASR(file, modelId, asrLanguage);
