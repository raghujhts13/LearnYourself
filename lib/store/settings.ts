...
const getDefaultAudioConfig = () => ({
  ...
  ttsProvidersConfig: {
    ...
    'puter-tts': { apiKey: '', baseUrl: '', enabled: true },
  ...
  asrProvidersConfig: {
    ...
    'puter-asr': { apiKey: '', baseUrl: '', enabled: true },
...
const getDefaultImageConfig = () => ({
  ...
  imageProvidersConfig: {
    ...
    'puter-seedream': { apiKey: '', baseUrl: '', enabled: true },
...
/**
 * Ensure ttsProvidersConfig and asrProvidersConfig include all known providers.
 * Called on every rehydrate so newly added providers (e.g. puter-tts, puter-asr)
 * appear automatically for existing users without clearing their saved configs.
 */
function ensureBuiltInAudioProviders(state: Partial<SettingsState>): void {
  const defaults = getDefaultAudioConfig();

  if (state.ttsProvidersConfig) {
    const defaultTTS = defaults.ttsProvidersConfig;
    for (const pid of Object.keys(defaultTTS) as TTSProviderId[]) {
      if (!state.ttsProvidersConfig[pid]) {
        state.ttsProvidersConfig[pid] = defaultTTS[pid];
      }
    }
  }

  if (state.asrProvidersConfig) {
    const defaultASR = defaults.asrProvidersConfig;
    for (const pid of Object.keys(defaultASR) as ASRProviderId[]) {
      if (!state.asrProvidersConfig[pid]) {
        state.asrProvidersConfig[pid] = defaultASR[pid];
      }
    }
  }
}
  ...
  persist(
        ...
        setImageGenerationEnabled: (enabled) => {
          if (enabled) {
            ...
            const hasUsable = Object.entries(cfg).some(([pid, c]) => {
              ...
              // 1. It's server-configured
              // 2. It has a client API key
              // 3. It doesn't require an API key at all (e.g. puter-seedream)
              const providerDef = IMAGE_PROVIDERS[pid as ImageProviderId];
              return c.isServerConfigured || c.apiKey || providerDef?.requiresApiKey === false;
            });
            if (!hasUsable) return;
          }
        ...
        fetchServerProviders: async () => {
          try {
            ...
            set((state) => {
              ...
              // Auto-disable image generation when no provider is usable
    ...
    },
    {
      ...
      migrate: (persistedState: unknown, version: number) => {
        ...
        // Ensure image configs have all built-in providers
        ensureBuiltInImageProviders(state);

        // Ensure audio configs have all built-in providers (puter-tts, puter-asr etc.)
        ...
        if (state.ttsModel && !state.ttsProviderId) {
          ...
          if (state.ttsModel === 'openai-tts') {
            ...
            // Default to OpenAI (removed providers like azure-tts map here)
            state.ttsProviderId = 'openai-tts';
          }
        }
