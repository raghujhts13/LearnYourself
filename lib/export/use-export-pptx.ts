 ...
 * Uses user-edited speakerNotes field if present, otherwise falls back to speech actions.
 */
function buildSpeakerNotes(scene: Scene): string {
  // Use user-edited notes if available
  if (scene.speakerNotes) {
    return scene.speakerNotes;
  }
...
/**
 * Build a full transcript of all scenes as a plain-text / Markdown document.
 */
export function buildTranscriptText(scenes: Scene[]): string {
  const lines: string[] = ['# Course Transcript', ''];
  for (const scene of scenes) {
    if (!scene.actions || scene.actions.length === 0) continue;
...
export function useExportPPTX() {
  ...
  const downloadTranscript = useCallback(() => {
    ...
    const fileName = stage?.name || 'transcript';
    const text = buildTranscriptText(scenes);
    const blob = new Blob([text], { type: 'text/markdown;charset=utf-8' });
    saveAs(blob, `${fileName}-transcript.md`);
  ...
  const exportResourcePack = useCallback(() => {
    withExportGuard(async () => {
      ...
      // [INTERACTIVE DISABLED] Interactive HTML pages export is disabled
      ...
      const transcriptText = buildTranscriptText(scenes);
      if (transcriptText.trim()) {
        zip.file(`${fileName}-transcript.md`, transcriptText);
      }
  ...
  return { exporting, exportPPTX, exportResourcePack, downloadTranscript };
}
