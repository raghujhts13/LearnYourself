 ...
 *
 * Generates full scenes (slide/quiz/pbl with actions; interactive is DISABLED)
 * from scene outlines.
 */
...
export async function generateSceneContent(
  ...
  | null
> {
  // [INTERACTIVE DISABLED] All interactive outlines fall back to slide
  if (outline.type === 'interactive') {
    log.warn(
      `Interactive outline "${outline.title}" disabled, falling back to slide`,
    );
...
async function generateSlideContent(
  ...
  const prompts = buildPrompt(PROMPT_IDS.SLIDE_CONTENT, {
    ...
    elements: '(auto-generated based on key points)',
...
// [INTERACTIVE DISABLED] Interactive learning functionality is disabled
// The generateInteractiveContent function has been commented out.
// All interactive outlines now fall back to slide generation.
/*
...
export async function generateSceneActions(
  ...
  if (outline.type === 'slide' && 'elements' in content) {
    ...
    if (actions.length > 0) {
      return processActions(actions, content.elements);
    }
 ...
 */
function processActions(actions: Action[], elements: PPTElement[]): Action[] {
