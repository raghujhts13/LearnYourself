...
- **Scene Types**: `slide` (presentation), `quiz` (assessment), and `pbl` (project-based learning) are supported
...
2. **type can be `"slide"`, `"quiz"`, or `"pbl"`**
3. **quiz type must include quizConfig**
4. **pbl type must include pblConfig** - with projectTopic, projectDescription, targetSkills, issueCount, and language
...
7. **Language Requirement**: Strictly output all content in the language specified by the user
