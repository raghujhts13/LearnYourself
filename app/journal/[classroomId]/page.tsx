...
export default function ClassroomJournalPage() {
  ...
  useEffect(() => {
    db.classrooms.get(classroomId).then((c) => {
      if (c) setClassroomName(c.name);
    });
  }, [classroomId]);
      ...
      <div className="shrink-0 px-6 py-4 border-b border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-bold text-gray-900 dark:text-white leading-tight">
            {classroomName || 'Journal'}
          </h1>
          {classroomName && (
            <p className="text-xs text-gray-400 dark:text-gray-500">My Journal</p>
          )}
        </div>
      </div>

      {/* Journal content filtered to this classroom */}
      <div className="flex-1 overflow-hidden">
        <JournalView classroomId={classroomId} />
      </div>
    </div>
  );
}
