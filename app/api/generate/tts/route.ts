...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Reject client-only TTS providers
    if (ttsProviderId === 'browser-native-tts' || ttsProviderId === 'puter-tts') {
      return apiError('INVALID_REQUEST', 400, `${ttsProviderId} must be handled client-side`);
    }
