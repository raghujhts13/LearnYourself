...
export function usePBLChat({ projectConfig, userRole, onConfigUpdate }: UsePBLChatOptions) {
  ...
  const kickedOffIssueIdsRef = useRef<Set<string>>(new Set());
  ...
  useEffect(() => {
    if (!currentIssue || isLoading) return;
    if (kickedOffIssueIdsRef.current.has(currentIssue.id)) return;

    const questionAgent = projectConfig.agents.find((a) => a.name === currentIssue.question_agent_name);
    if (!questionAgent) return;

    kickedOffIssueIdsRef.current.add(currentIssue.id);
    ...
    const kickoff = async () => {
      ...
      try {
        ...
        const kickoffPrompt = [
          `You are opening this issue for the student role "${userRole}".`,
          `Issue: ${currentIssue.title}`,
          `Provide a concise interactive kickoff:`,
          `1) explain the immediate objective in plain language`,
          `2) ask one concrete first-step question the student should answer next`,
        ].join('\n');
        ...
        const response = await fetch('/api/pbl/chat', {
          ...
          body: JSON.stringify({
            ...
            agent: questionAgent,
            currentIssue,
            recentMessages: projectConfig.chat.messages.slice(-10).map((m) => ({
              agent_name: m.agent_name,
              message: m.message,
            })),
            userRole,
          ...
          onConfigUpdate({
            ...
            chat: {
              messages: [
                ...
                {
                  id: `msg_${Date.now()}_kickoff`,
                  agent_name: questionAgent.name,
                  message: data.message,
                  timestamp: Date.now(),
                  read_by: [],
                },
        ...
        log.error('[usePBLChat] Auto-kickoff failed:', error);
      } finally {
        if (!cancelled) {
          setIsLoading(false);
        }
  ...
  }, [currentIssue, isLoading, onConfigUpdate, projectConfig, userRole]);
