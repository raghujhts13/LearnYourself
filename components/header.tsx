...
interface HeaderProps {
  ...
  readonly onToggleSlideEditing?: () => void;
...
export function Header({
  ...
  // Export
  const { exporting: isExporting, exportPPTX, exportResourcePack, downloadTranscript } = useExportPPTX();
    ...
    <>
      <header className="h-20 px-8 flex items-center justify-between z-10 bg-transparent gap-4">
        ...
        <div className="flex items-center gap-4 bg-white/60 dark:bg-gray-800/60 backdrop-blur-md px-2 py-1.5 rounded-full border border-gray-100/50 dark:border-gray-700/50 shadow-sm shrink-0">
          {/* Slide Edit Mode Toggle */}
          {onToggleSlideEditing && (
            <button
              onClick={onToggleSlideEditing}
              title={isSlideEditing ? 'Switch to view mode' : 'Edit this slide'}
              className={cn(
            ...
            >
              {isSlideEditing ? <Eye className="w-4 h-4" /> : <Pencil className="w-4 h-4" />}
            </button>
          )}
          ...
          {onToggleQASidebar && (
            <button
              ...
              title={showQASidebar ? 'Close Q&A assistant' : 'Open Q&A assistant'}
              className={cn(
                'p-2 rounded-full transition-all',
                showQASidebar
            ...
            <div className="absolute top-full mt-2 right-0 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg overflow-hidden z-50 min-w-[200px]">
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-[11px] text-gray-400 dark:text-gray-500">
                    Full lecture script as Markdown
                  </div>
                </div>
              </button>
            </div>
