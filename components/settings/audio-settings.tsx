...
import { Volume2, Mic, MicOff, CheckCircle2, XCircle, Eye, EyeOff, Info } from 'lucide-react';
...
function getTTSProviderName(providerId: TTSProviderId, t: (key: string) => string): string {
  const names: Partial<Record<TTSProviderId, string>> = {
    ...
    'puter-tts': t('settings.providerPuterTTS'),
  };
  return names[providerId] ?? TTS_PROVIDERS[providerId]?.name ?? providerId;
}
...
function getASRProviderName(providerId: ASRProviderId, t: (key: string) => string): string {
  const names: Record<ASRProviderId, string> = {
    ...
    'puter-asr': t('settings.providerPuterASR'),
  };
...
export function AudioSettings({ onSave }: AudioSettingsProps = {}) {
  ...
  useEffect(() => {
    const availableVoices = getTTSVoices(ttsProviderId);
  ...
  }, [ttsProviderId, ttsVoice, setTTSVoice]);
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="space-y-4">
        ...
        >
          ...
          {ttsProviderId === 'puter-tts' && (
            <div className="rounded-lg bg-blue-50/50 dark:bg-blue-950/20 px-4 py-3 border border-blue-100 dark:border-blue-900/30 mb-4">
              <div className="flex gap-2">
                <Info className="h-4 w-4 text-blue-500 shrink-0 mt-0.5" />
                <div className="space-y-1">
                  <p className="text-sm font-medium text-blue-900 dark:text-blue-200">
                    {t('settings.freeNoKeyRequired')}
                  </p>
                  <p className="text-xs text-blue-700/80 dark:text-blue-300/80 leading-relaxed">
                    {t('settings.puterFreeNotice')}
                  </p>
                </div>
              </div>
            </div>
