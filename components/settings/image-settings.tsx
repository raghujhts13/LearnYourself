...
export function ImageSettings({ selectedProviderId }: ImageSettingsProps) {
  ...
  const isKeyless = currentProvider?.requiresApiKey === false;
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {/* Puter.js keyless notice */}
      {isKeyless && (
        <div className="rounded-lg border border-emerald-200 bg-emerald-50 dark:border-emerald-800 dark:bg-emerald-950/30 p-3 text-sm text-emerald-700 dark:text-emerald-300 space-y-1">
          <p className="font-semibold">✨ Free — No API Key Required</p>
          <p>
            This provider uses{' '}
            <a
            ...
            to run image generation directly in your browser at no cost.
            On first use, you may be asked to log in to Puter.com (free account).
          </p>
        </div>
      )}

      {/* API Key + Test inline — hidden for keyless providers */}
      {!isKeyless && (
      ...
      {/* Base URL — hidden for keyless providers */}
      {!isKeyless && (
        <div className="space-y-2">
